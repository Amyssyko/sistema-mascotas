...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
	...
	const { name, type, breed, photo, description } = json
	...
	try {
		...
		const { error } = schema.validate({ id, name, type, age, month, breed, photo, description })
		if (error) {
			return new NextResponse(error.message, { status: 400 })
		}
		const updatedPet = await prisma.pet.update({
			where: { id },
			data: { name, type, age, month, breed, photo, description },
		})
