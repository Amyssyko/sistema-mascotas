...
export default function Page() {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const response = await fetch("http://localhost:3000/api/v2/adoptions")
					...
					<div className="mb-8 flex items-center justify-between gap-8">
						<div>
							<Typography variant="h5" color="blue-gray">
								Lista de Solicitudes de Adopción de Mascotas
							</Typography>
				...
				<CardBody className="overflow-scroll px-0">
					<table className="mt-4 w-full min-w-max table-auto text-left">
						...
						<tbody>
							{pets.map(
								(
									{ id, petId, jobType, income, status, description, personDni, createdAt, updatedAt },
									index
								) => {
										...
										<tr key={id}>
											...
											<td className={classes}>
												<Button
													color="green"
													size="sm"
													onClick={() => router.replace(`/dashboard/peticiones/${id}`)}
												>
