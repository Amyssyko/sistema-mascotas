...
export default function Page() {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const response = await fetch("http://localhost:3000/api/v2/lostpets")
	...
	const handleDelete = async ({ id }: { id: Number }) => {
		try {
			const response: AxiosResponse = await axios.post(`/api/v2/lostpets/${id} `)
			...
			<Card className="h-full w-full">
					...
					<div className="mb-8 flex items-center justify-between gap-8">
						<div>
							<Typography variant="h5" color="blue-gray">
								Lista de Mascotas Perdidas y Encontradas
							</Typography>
							<Typography color="gray" className="mt-1 font-normal">
								Informacion de todos las mascotas registradas
							</Typography>
						</div>
						<div className="flex shrink-0 flex-col gap-2 sm:flex-row">
							<Button className="flex items-center gap-3" color="blue" size="sm">
								...
								<Link href="/dashboard/perdido">Nuevo Regisrtro</Link>
							</Button>
						</div>
					</div>
				</CardHeader>
				<CardBody className="overflow-scroll px-0">
					<table className="mt-4 w-full min-w-max table-auto text-left">
						...
						<tbody>
							{pets.map(({ id, lostLocation, status, name, createdAt, updatedAt, description }, index) => {
									...
									<tr key={name}>
										...
										<td className={classes}>
											<Button
												color="green"
												size="sm"
												onClick={() => router.replace(`/dashboard/perdido/${id}`)}
											>
