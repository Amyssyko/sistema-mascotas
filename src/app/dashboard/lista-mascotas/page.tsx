...
export default function Page() {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const response = await fetch("http://localhost:3000/api/v2/pets")
			...
			<Card className="h-full w-full">
					...
					<div className="mb-8 flex items-center justify-between gap-8">
						...
						<div className="flex shrink-0 flex-col gap-2 sm:flex-row">
							<Button className="flex items-center gap-3" color="blue" size="sm">
								...
								<Link href="/dashboard/mascotas">Nueva Mascota</Link>
							</Button>
						</div>
					</div>
				</CardHeader>
				<CardBody className="overflow-scroll px-0">
					<table className="mt-4 w-full min-w-max table-auto text-left">
						...
						<tbody>
							{pets.map(({ id, name, typePet, age, month, breed, photo, description }, index) => {
									...
									<tr key={name}>
										...
										<td className={classes}>
											<Button
												color="green"
												size="sm"
												onClick={() => router.replace(`/dashboard/mascotas/${id}`)}
											>
